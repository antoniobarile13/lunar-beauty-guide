...
export default function CategoryOrder() {
  ...
  const handleCategorySelect = (category: BeautyCategory) => {
    ...
    const favorable = adviceRange
      .filter(advice => advice.items[category].score >= 1)
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-white/80">
              {t("categoryOrder.favorableDatesFor", { category: t(`categories.${selectedCategory}`) })}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {favorableDates.length === 0 ? (
            ...
            favorableDates.map((advice) => (
              <Card 
                key={advice.dateISO}
                className="bg-gradient-card/80 backdrop-blur-sm border-border/50 hover:shadow-soft transition-smooth cursor-pointer"
                onClick={() => navigate(`/day/${advice.dateISO}`)}
              >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-center">
                        ...
                        <div className="text-xs text-muted-foreground uppercase">
                          {new Date(advice.dateISO).toLocaleDateString('it-IT', { month: 'short' })}
                        </div>
                      </div>
                      <div>
        ...
        {categories.map((categoryData) => {
          ...
          return (
            <Card
              ...
              className="bg-gradient-card/80 backdrop-blur-sm border-border/50 hover:shadow-soft transition-smooth cursor-pointer group"
